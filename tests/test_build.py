...
@pytest.mark.parametrize(
    ("relative_dir", "expected_content"),
    [
        (None, "This is a static file."),
        ("subdir", "This is a static file in a subdirectory."),
    ],
...
def test_fetch_content_files(tmp_path: Path) -> None:
    ...
    assert len(files) == 2
    assert isinstance(files[0], t.DocumentFile)
    assert files[0] == doc_file
    assert isinstance(files[1], t.StaticFile)
    assert files[1] == static_file
